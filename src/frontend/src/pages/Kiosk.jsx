...
export default function Kiosk() {
  ...
  // Auto-print when ticket is created
  useEffect(() => {
    if (step !== STEP.DONE || !ticket) return;
    setShowReceipt(true);
    const printTimer = setTimeout(() => window.print(), 600);
    // Fallback reset if print dialog never fires afterprint
    fallbackRef.current = setTimeout(reset, 45000);
    return () => { clearTimeout(printTimer); clearTimeout(fallbackRef.current); };
  }, [step, ticket]);

  // Reset to welcome when print dialog closes (print or cancel)
  useEffect(() => {
    const handler = () => { clearTimeout(fallbackRef.current); setTimeout(reset, 1500); };
  ...
  return (
    <>
      {/* Print/screen visibility control */}
      <style>{`
        @media print  { .kiosk-screen  { display: none !important; } }
        @media screen { .kiosk-receipt { display: none !important; } }
      `}</style>

      {/* ── Kiosk UI (screen only) ─────────────────────────────────────── */}
      <div className="kiosk-screen min-h-screen bg-navy flex flex-col select-none"
        ...
        <header className="px-8 py-5 flex items-center justify-between border-b border-white/10 shrink-0">
          ...
          {step > STEP.WELCOME && step < STEP.DONE && (
            <button onClick={reset} className="text-white/40 text-lg hover:text-white/70 transition-colors px-3 py-1">
              ✕ Cancel
            </button>
          )}
        </header>
        ...
        <div className="flex-1 flex items-center justify-center p-8">
          ...
          {step === STEP.DEPARTMENT && (
            <div className="w-full max-w-2xl">
              ...
              {submitting && <p className="text-white/60 text-center mt-8 text-xl animate-pulse">Creating your ticket…</p>}
            </div>
          )}
          ...
          {step === STEP.CATEGORY && (
            <div className="w-full max-w-2xl">
              ...
              {submitting && <p className="text-white/60 text-center mt-6 text-xl animate-pulse">Creating your ticket…</p>}
            </div>
          )}
          ...
          {step === STEP.DONE && ticket && (
            <div className="text-center w-full max-w-lg">
              ...
              <p className="text-white/60 text-xl animate-pulse">🖨 Printing your ticket…</p>
              <p className="text-white/30 text-sm mt-3">
                Please collect your ticket from the printer and wait in the reception area.
              </p>
            </div>
          )}
        </div>
